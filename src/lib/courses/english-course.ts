...
const GRAMMAR_BY_LEVEL: Record<string, { title: string; explanation: string; rules: string[]; grammar: { sentence: string; options: string[]; correct: string; explanation: string } }[]> = {
  B1: [
    ...
    { title: 'Question Formation', explanation: 'Form clear questions with question words and auxiliary verbs.', rules: ['Yes/No questions: do/does/did + subject + base verb', 'Wh-questions start with what/where/when/why/who/how', 'Invert the subject and the auxiliary verb'], grammar: { sentence: '___ do you usually go on holiday?', options: ['Where', 'Who', 'Which', 'Whose'], correct: 'Where', explanation: 'Asking about a place → Where.' } },
  ],
  'B1+': [
    ...
    { title: 'Common Phrasal Verbs', explanation: 'Verbs combined with particles whose meaning differs from the base verb.', rules: ['give up = quit / stop trying', 'look after = take care of', 'find out = discover'], grammar: { sentence: 'Please ___ your shoes before you come in.', options: ['take off', 'take on', 'take up', 'take in'], correct: 'take off', explanation: 'Remove clothing → take off.' } },
  ],
  B2: [
